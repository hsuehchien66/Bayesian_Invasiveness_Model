...
parameters {
  ...
  // log probability to disease (psi) ~ U(-5,1)
  vector<lower=-5, upper=0.0>[j_max] log_psi_j;
  
  // carriage duration  10 days - 292 days
  vector<lower=0.03, upper=0.8>[j_max] dur_j;
...
transformed parameters {
  ...
  */
  ...
  // calculate probability to disease on a real scale
  vector<lower=0,upper=100.0>[j_max] psi_j;
  for (j in 1:j_max) {
    psi_j[j] = pow(10, log_psi_j[j]);
  }
...
model {
  ...
  */
  ...
  // Calculate prior probability for probability to disease
  for (j in 1:j_max) {
    target += uniform_lpdf(log_psi_j[j] | -5, 0);
  }

  // Calculate prior probability for carriage duration
  for (j in 1:j_max) {
    target += normal_lpdf(dur_j[j] | 0.08, 0.01);
  }
  ...
  for (index in 1:n_obs) {
    ...
    target += poisson_lpmf(d_ij[index] | gamma_i[i]*(psi_j[j]/dur_j[j])*rho_ij[index]*N_i[index]*t_i[index]);
...
generated quantities {
  ...
  for (index in 1:n_obs) {
    ...
    d_ij_pred[index] = gamma_i[i]*(psi_j[j]*dur_j[j])*rho_ij[index]*N_i[index]*t_i[index];
